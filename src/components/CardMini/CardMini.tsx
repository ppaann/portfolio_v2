        ...
        <div className='flex space-x-2'>
          {tags &&
            tags.map((tag, index) => {
              const tagColor =
                index % 3 === 0
                  ? 'text-gray-700 bg-gray-200 dark:bg-gray-800 dark:text-gray-200'
                  : index % 3 === 1
                  ? 'text-cyan-600 bg-cyan-200 dark:bg-cyan-800 dark:text-cyan-200'
                  : 'text-purple-600 bg-purple-200 dark:bg-purple-800 dark:text-purple-200';

              return (
                <span
                  key={index}
                  className={`text-xs font-semibold inline-block py-1 px-2 uppercase rounded-full ${tagColor}`}
                >
                  {tag}
                </span>
              );
